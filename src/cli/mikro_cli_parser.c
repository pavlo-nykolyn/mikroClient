...
/* CLI argument */
   mikro_String_t* item_name; // will not include the prefix --
   mikro_String_t* item_value;
};
                                      ...
                                      int* restrict mikro_pCd)
{
   ...
   mikro_Cli_item_t* pItem = (mikro_Cli_item_t*) calloc(1, sizeof(struct mikro_cli_item));
   if (!pItem) {
      mikro_Log_append(stdout, __LINE__ - 2, "progErr", mikro_messages[mikro_ind_heapFail]);
      exit(EXIT_FAILURE);
   }
   // allocating space for the name
   mikro_FastString_t keyFStr = {.len = kSz,
                                 .pChArr = (mikro_pFS -> pChArr) + off};
   pItem -> item_name = mikro_String_create(&keyFStr,
                                            &cd);
   if (!(pItem -> item_name))
   ...
   if (kSz < (mikro_pFS -> len) - off) {
      ...
      mikro_FastString_t valFStr = {.len = vSz,
                                    .pChArr = (mikro_pFS -> pChArr) + off + kSz + 1};
      pItem -> item_value = mikro_String_create(&valFStr,
                                                &cd);
      if (!(pItem -> item_value))
         goto MIKRO_CLI_PARSE_ARG_EXIT;
   }
...
{
   if (mikro_addrItem) {
      mikro_Cli_item_t* tmp = *mikro_addrItem;
      if (tmp -> item_name)
